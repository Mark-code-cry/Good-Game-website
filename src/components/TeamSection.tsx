  ...
  className="bg-[#0d1113] border border-[#00bcd4]/40 rounded-2xl overflow-hidden shadow-[0_0_10px_#00bcd455] w-[100px] sm:w-[110px] md:w-36 lg:w-40 transition-all duration-300"
  >
  <img src={member.image} alt={member.name} className="w-full h-24 sm:h-28 md:h-36 object-cover" />
  <div className="p-1 bg-[#00bcd4]/10 text-center min-h-[30px] md:min-h-[50px]">
  <p className="text-[8px] sm:text-[9px] md:text-xs font-semibold text-[#26eeeb] leading-tight whitespace-nowrap overflow-hidden text-ellipsis">
  {member.name}
  </p>
  <p className="text-[8px] sm:text-[9px] md:text-[10px] text-gray-300 leading-tight">{member.role}</p>
  </div>
  </motion.div>
  );

  return (
  <section className="min-h-screen flex flex-col justify-center bg-gradient-to-b from-[#222222] to-[#222222] text-white py-8 -mt-56 sm:-mt-64 md:-mt-72 lg:mt-0 xl:mt-0">
  ...
  <motion.div key="category" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
  ...
  >
  {winConditionMet ? (
  <h3 className="text-3xl font-bold text-green-500">You're Good! 🏆</h3>
  ) : (
  ...
  >
  {category === "picture" ? (
  <img src={option} alt="Option" className="w-full h-24 object-cover rounded-md" />
  ) : (
  <span>{option}</span>
  )}
  </button>
